 ...
 */
typedef struct {
    uint16_t version_be; // 2 字节版本（网络序）
    uint16_t length_be;  // 2 字节长度（网络序）
    uint8_t  flags_raw;  // 1 字节标志
} proto_header_raw_t;
 ...
 */
typedef struct {
    uint8_t  ver_major;  // 4 位主版本
    uint8_t  ver_minor;  // 4 位次版本
    uint16_t length;     // 16 位无符号
    uint8_t  flags;      // 5 位功能标志
} proto_header_bits_t;
...
int main(void) {
    ...
    uint16_t ver_be = be16_to_cpu(raw.version_be);
    uint8_t major = (ver_be >> 4) & 0x0F;
    uint8_t minor = ver_be & 0x0F;
