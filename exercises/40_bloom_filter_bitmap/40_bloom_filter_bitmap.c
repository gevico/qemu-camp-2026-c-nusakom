...
static size_t hash_k(const char *s, size_t m, int k) {
    size_t hash = 0;
    while (*s) {
        hash = (hash * (size_t)k + (unsigned char)*s) % m;
        s++;
    }
    return hash;
}
...
static void bloom_add(Bloom *bf, const char *s) {
    set_bit(bf->bits, hash_k(s, bf->m, 31));
    set_bit(bf->bits, hash_k(s, bf->m, 37));
    set_bit(bf->bits, hash_k(s, bf->m, 41));
}

static int bloom_maybe_contains(Bloom *bf, const char *s) {
    if (!test_bit(bf->bits, hash_k(s, bf->m, 31))) return 0;
    if (!test_bit(bf->bits, hash_k(s, bf->m, 37))) return 0;
    if (!test_bit(bf->bits, hash_k(s, bf->m, 41))) return 0;
    return 1;
}
